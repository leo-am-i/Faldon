...
void AntiMagic(SocketSender& sender, int delay) {
    //Implement a char id finder to dynamically get your char id
    int charID = 0;
    unsigned char antimagic[] = { 0x84, 0x0F, 0x41, 0x01, 0x00, 0x00 };
    antimagic[4] = static_cast<unsigned char>(charID);
    
    while (true) {
        sender.send_packet(antimagic, sizeof(antimagic));
...
int main() {
    ...
    std::cout << "Enter the delay between casts in seconds: ";
    std::cin >> delay;

    std::cout << "Press enter to start the script" << std::endl;
    system("pause");

    system("cls");
    std::cout << "Running Antimagic script by clear-man" << std::endl;
    std::cout << "Faldon.Client.exe - Process ID: " << client_pid << std::endl;
    if(delay>1)
    std::cout << "You are casting Antimagic every " << delay << " seconds" << std::endl;
    	else
        std::cout << "You are casting Holy Armour every " << delay << " second" << std::endl;
    std::cout << "Close this window to exit" << std::endl;

    std::thread start_thread(AntiMagic, std::ref(sender), charID, delay);
    start_thread.join();
